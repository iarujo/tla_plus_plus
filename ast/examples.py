...
def test_one_round_honest_quorum():
    ...
    Init = Definition("Init", Conjunction([network == Set([]), decided == RecordInstance([In(a, Acceptor)], [Scalar(-1)])]))
...
def test_byzantine_quorum_infinite_tla_plusplus():
    ...
    f.write(repr(spec.compile()))
    f.close()

def test_leader():
    """ Ensure a simple spec with a byzantine leader is printed and compiled correctly """
    ...
    Acceptors = Constant("Acceptors")
    Values = Constant("Values")
    consts = Constants([MaxByzantineNodes, Acceptors])
    
    vars = Variables([king])
    
    w = Alias("w", None)
    ByzLeader = Definition(
        name="byzleader",
        value=ByzantineLeader(
            hon_behaviour=ExistentialQuantifier(
                variables=[w],
                set=Values,
                predicate= Alias("Broadcast", [w])
            ),
            byz_behaviour=Alias("Broadcast", [w])
        )
    )
    
    spec = Spec(module="MySpec", extends=extends, constants=consts, assumptions=None, variables=vars, defs=[ByzLeader])
    
    print(repr(spec))
    print(repr(spec.compile()))
